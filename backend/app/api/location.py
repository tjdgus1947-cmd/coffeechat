...
# ⭐️ 3. 주소 검색 API (Kakao Local API) ⭐️
@router.get("/api/locations/search-address")
def search_address(query: str):
    """
    Kakao Local API를 사용하여 주소를 검색하고 좌표를 반환합니다.
    """
    try:
        import requests
        import os
        ...
        if not KAKAO_API_KEY:
            raise HTTPException(status_code=500, detail="Kakao API 키가 설정되지 않았습니다.")
        ...
        raise HTTPException(status_code=500, detail=f"주소 검색 실패: {str(e)}")
